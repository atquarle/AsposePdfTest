...
internal class Program
{
    static void Main()
    {
        ...
        var licenseFileName = $"{assembly.GetName().Name}.Aspose.Total.NET.lic";

        using var stream = assembly.GetManifestResourceStream(licenseFileName);
        pdfLicense.SetLicense(stream);

        var outputDirectory = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ProgramOutput");
        Directory.CreateDirectory(outputDirectory);

        var path = Path.Combine(outputDirectory, "Output.pdf");
